...
// Import the shared ProgressCallback interface
...
public class ExcelFileComparator {
	...
	// Method to compare Excel files in directories based on the new matching logic
	...
	public static void compareSpecificFiles(File sourceFile, File generatedFile, String outputDirPath,
		...
		if (generatedFile == null) {
			throw new IllegalArgumentException("Generated file cannot be null.");
		}
		// Call the comparison method directly
		compareAndHighlightDifferences(sourceFile, generatedFile,
	...
	public static void compareSpecificFiles(File sourceFile, File generatedFile, String generatedDirPath,
			String outputDirPath, ProgressCallback callback) throws IOException {
		if (generatedFile != null) {
			...
			for (File generatedFileMatch : matchingGeneratedFiles) {
				...
				callback.log("Queuing comparison for source file: " + sourceFile.getName() + " with generated file: "
						+ generatedFileMatch.getName());
				CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
					try {
	...
	// public static void compareSpecificFiles(File sourceFile, File generatedFile,
	// String outputDirPath,
	// ProgressCallback callback) throws IOException {
	// compareAndHighlightDifferences(sourceFile, generatedFile,
	// Paths.get(outputDirPath, "Compared_" + generatedFile.getName()).toString(),
	// callback);
	// }
	...
	// private static File findMatchingSourceFile(Map<String, File> sourceFiles,
	...
	private static class RowData {
		...
		public RowData(Row row, List<Integer> keyColumns) {
			...
			if (keyColumns != null) {
				...
				// Debug log
				System.out.println("Generated key for sheet '" + row.getSheet().getSheetName() + "' at row "
						+ (row.getRowNum() + 1) + ": " + this.key);
			} else {
				this.key = null;
			}
		}
	...
	public static void main(String[] args) {
		try {
            ...
            ProgressCallback callback = message -> System.out.println(message);

			compareExcelFiles(sourceDirPath, generatedDirPath, outputDirPath, callback);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
